...
export function VoucherSelection({
    ...
    const fetchCustomerVouchers = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const response = await customerService.getCustomerVoucherByCustomerId(customerId);
    ...
    const filteredVouchers = vouchers.filter(voucher =>
        voucher.voucher.voucherCode.toLowerCase().includes(searchCode.toLowerCase()) ||
        voucher.voucher.voucherName.toLowerCase().includes(searchCode.toLowerCase())
    );
...
function VoucherCard({ customerVoucher, onSelect }: VoucherCardProps) {
    ...
    const isExpiringSoon = new Date(voucher.expirationDate) <= new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);

    return (
        <div
            className="cursor-pointer rounded-lg border border-gray-200 p-4 transition-colors hover:border-primary hover:bg-primary/5"
            onClick={onSelect}
        >
            <div className="flex items-center justify-between">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mb-1">Mã: {voucher.voucherCode}</p>
