...
// Hooks and Types
import { useVaccineBatchUpdate } from "../hooks/useVaccineBatchUpdate";
...
export function VaccineBatchUpdateForm({
  vaccineBatch,
  onSuccess,
  onCancel,
}: Props) {
  const { mutate: updateVaccineBatch, isPending } = useVaccineBatchUpdate();
  ...
  const form = useForm<VaccineBatchFormData>({
    ...
    defaultValues: {
      ...
      manufacturer: vaccineBatch.manufacturer || "", // Use actual value from vaccineBatch
      source: vaccineBatch.source || "", // Use actual value from vaccineBatch
      storageCondition: vaccineBatch.storageCondition || "", // Use actual value from vaccineBatch
    },
  });

  // Reset form when vaccineBatch changes
  useEffect(() => {
    if (vaccineBatch) {
      form.reset({
        vaccineId: vaccineBatch.vaccineId,
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-6">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? "Đang cập nhật..." : "Cập nhật lô vaccine"}
          </Button>
        </div>
      </form>
    </Form>
