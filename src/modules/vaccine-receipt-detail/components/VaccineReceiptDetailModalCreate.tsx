...
// hooks
import { useCreateVaccineReceiptDetail } from "../hooks/useCreate";
...
interface VaccineReceiptDetailModalCreateProps {
  open: boolean;
...
export function VaccineReceiptDetailModalCreate({
  open,
  onOpenChange,
  vaccineReceiptId,
  receiptCode,
}: VaccineReceiptDetailModalCreateProps) {
  ...
  const { mutate: createDetail, isPending } = useCreateVaccineReceiptDetail();
  ...
  const handleSubmit = (data: VaccineReceiptDetailCreateFormData) => {
    // Debug logging
    console.log("Form data:", data);
    console.log("VaccineReceiptId from props:", vaccineReceiptId);

    // Validate vaccineReceiptId
    if (!vaccineReceiptId || vaccineReceiptId === 0) {
      toast.error("Lỗi", {
        description: "ID phiếu nhập không hợp lệ. Vui lòng thử lại.",
      });
      return;
    }

    // Validate vaccineBatchId
    if (!data.vaccineBatchId || data.vaccineBatchId === 0) {
      toast.error("Lỗi", {
    ...
    const payload = {
      vaccineReceiptId: vaccineReceiptId, // Use prop value to override form value
      ...
      coldChainLog: {
        vaccineBatchId: data.vaccineBatchId,
        logTime: data.coldChainLog.logTime,
        temperature: data.coldChainLog.temperature,
        humidity: data.coldChainLog.humidity,
        event: data.coldChainLog.event,
        notes: data.coldChainLog.notes || "",
      },
    };
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-primary">
            Thêm vaccine vào phiếu nhập
          </DialogTitle>
          <DialogDescription>
            Thêm thông tin vaccine chi tiết vào phiếu nhập {receiptCode}
          </DialogDescription>
        </DialogHeader>
          ...
          >
            ...
            <DialogFooter className="gap-2 pt-4">
              ...
              <Button type="submit" disabled={isPending}>
                {isPending ? "Đang thêm..." : "Thêm vaccine"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
